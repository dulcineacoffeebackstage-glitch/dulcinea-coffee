/* ============================================================
   杜辛亞咖啡 Dulcinea Coffee — 連線設定
   ------------------------------------------------------------
   ⚠️ 這是給杜辛亞咖啡用的新 Supabase 專案，還沒填金鑰。
   請到 Supabase 開一個全新的專案（不要跟別家店共用），
   在 Project Settings → API 找到下面兩樣東西，貼進來：

   1) Project URL（長得像 https://abcdefghijklmn.supabase.co）
   2) Publishable key（sb_publishable_ 開頭）

   填好之後，記得依序到 SQL Editor 執行一次：
   schema.sql → member-coupons.sql → site-settings.sql → storage-setup.sql
